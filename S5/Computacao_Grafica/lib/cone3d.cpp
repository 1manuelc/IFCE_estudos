...
class Cone {
    private:
        ...
        vector<vector<Vertex>> drawConeLateralFaces(Vertex top_vtx, vector<Vertex> base_vtxs) {
            ...
            for(int i = 0; i < base_vtxs.size(); i++) {
                ...
                vector<Vertex> face_vtxs = {
                    ...
                    Vertex("vtx_" + std::to_string(i) + "_2",
                        base_vtxs[next].get_x(),
                        base_vtxs[next].get_y(),
                        base_vtxs[next].get_z(),
                        {0.9, 0.9, 0.9})
                };
    ...
    public:
        Cone(int baseSegments_n, float baseRadius_n, Vertex top_vtx_n, double z_floor_n) {
        ...
        void render3d(GLenum renderPrimitive) {
            ...
            for(int i = 0; i < lateral_face_vtxs.size(); i++) {
                for(int j = 0; j < lateral_face_vtxs[0].size(); j++) {
                    glColor3f(lateral_face_vtxs[i][j].get_color()[0], lateral_face_vtxs[i][j].get_color()[1], lateral_face_vtxs[i][j].get_color()[2]);
                    glVertex3f(lateral_face_vtxs[i][j].get_x(), lateral_face_vtxs[i][j].get_y(), lateral_face_vtxs[i][j].get_z());
                }
            }
            glEnd();
        }
